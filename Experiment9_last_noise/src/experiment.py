...
class Intervention():
    ...
    def __init__(self,
        ...
        self.base_strings_tok = [
            ...
            # noise_string(include padding token at left side)
            {
        ...
        # noise_position = noise_sent.split().index(verb)-1
        # print(self.enc.decode(strings_token['input_ids'][0][clear_position]))
        # print(self.enc.decode(strings_token['input_ids'][1][noise_position]))
        self.position = (clear_position,noise_position)
...
class Model():
    ...
    def neuron_intervention(self,
        ...
        def intervention_hook(module,
                              ...
                              output, 
                              # subject의 마지막 토큰의 위치(clear, noise)
                              position,
            ...
            noise_position = position[1]
            # print("base_position: ",base_position)
            noise_slice = self.order_dims((slice(None), noise_position, slice(None)))

            # take a representation on the base_position
            # and gather them where position is neurons.
            base = output[noise_slice].gather(1, neurons)
            ...
            for i, v in enumerate(neurons):
                ...
                scatter_mask[self.order_dims((i, noise_position, v))] = 1
    ...
    def neuron_intervention_single_experiment(self,
        ...
        with torch.no_grad():
            ...
            noise_representations = self.get_representations(
                intervention.base_strings_tok[1], # noise sent e.g. {'input_ids': ~~,'attention_mask':~~}
            ...
            # self.num_layers = 12, self.num_neurons = 768
                ...
                context = intervention.base_strings_tok[1] # noise input
                ...
                for layer in range(-1, self.num_layers):
                    ...
                    for neurons in batch(range(self.num_neurons), bsize):
                        ...
                        patched_layer = self.neuron_intervention(
                            ...
                            # padding포함 verb 이전위치
                            # position = (clear subject position, noise position)
