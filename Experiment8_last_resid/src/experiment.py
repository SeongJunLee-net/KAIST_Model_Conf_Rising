...
class Model():
    def __init__(self,
        ...
        if self.is_gpt2:
            ...
            self.neuron_layer = lambda layer: self.model.transformer.h[layer]
    ...
    def get_representations(self, context, position):
        ...
        def extract_representation_hook(module,
                                        ...
                                        representations,
                                        layer):
            # wte와 일반 hidden layer사이에 차원이 하나가 차이남(e.g. wte: [9,768], layer: [1,9,768])
            # output이 tuple로 떨어지는데, output[0]가 hidden state고 output[1]은 cache
            if output[0].dim()==2:
                representations[layer] = output[0][self.order_dims((position,slice(None)))]
            elif output[0].dim()==3:
                representations[layer] = output[0][self.order_dims((0,position,slice(None)))]
            else:
    ...
    def neuron_intervention(self,
        ...
        def intervention_hook(module,
            ...
            if type(module) == torch.nn.modules.sparse.Embedding:
                base = output[entity_slice].gather(1, neurons)
                # Overwrite values in the output
                # First define mask where to overwrite
                scatter_mask = torch.zeros_like(output, dtype=torch.bool)
            else:
                base = output[0][entity_slice].gather(1, neurons)
                scatter_mask = torch.zeros_like(output[0], dtype=torch.bool)
            ...
            if type(module) == torch.nn.modules.sparse.Embedding:
                ...
                output[0].masked_scatter_(scatter_mask, base.flatten())
    ...
    def edit_representation(self,edit_data,intervention,layers,method):
        ...
        def edit_hook(module,
            ...
            if type(module) == torch.nn.modules.sparse.Embedding:
                ...
                output[0][clear_slice] = output[0][clear_slice] + rep
